...
if TYPE_CHECKING:
    ...
    from fancode.client import LLMClient
...
class AgentTool(Tool):
    ...
    def _inherited_rule_engine(self) -> Any:
        ...
        父级没有权限检查器时退化为空规则集。
        """
        from fancode.permissions import RuleEngine
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from fancode.agents.fork import ForkError, build_forked_messages
        from fancode.agents.parser import AgentDef
        from fancode.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from fancode.agent import Agent as AgentClass
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from fancode.agents.fork import ForkError, build_forked_messages
        from fancode.agents.parser import AgentDef
        from fancode.agents.tool_filter import build_teammate_tools
        from fancode.agent import Agent as AgentClass
        from fancode.conversation import ConversationManager
        from fancode.permissions import (
            DangerousCommandDetector,
        ...
        from fancode.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        from fancode.teams.spawn import build_teammate_cli
        ...
        if mailbox is not None and p.prompt:
            ...
            from fancode.teams.spawn_inprocess import LEAD_NAME
            mailbox.write(
                teammate_name,
                create_message(
        ...
        # 构造把本 fancode 拉起为队友 worker 模式的命令，cd 到该队友的 worktree
        cli_command = build_teammate_cli(p.team_name, teammate_name, wt.path)

        try:
            if backend == BackendType.TMUX:
                from fancode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from fancode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from fancode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from fancode.client import create_client
