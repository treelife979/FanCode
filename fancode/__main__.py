...
from fancode.config import ConfigError, load_config
from fancode.hooks import HookConfigError, HookEngine, load_hooks
from fancode.permissions import PermissionMode


def main() -> None:
    # 队友 worker 模式：由 tmux/iTerm2 窗格用 `-m fancode --teammate ...` 拉起。
    ...
    # 先确保 .fancode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".fancode").mkdir(parents=True, exist_ok=True)
    # 追加写：排查异常退出要看的往往是上一次运行的日志，覆盖会把现场冲掉
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".fancode/debug.log",
        filemode="a",
    )
    crashlog.install()

    parser = argparse.ArgumentParser(prog="fancode", description="FanCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from fancode.remote import RemoteServer
    ...
    from fancode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from fancode.client import create_client, resolve_context_window
    from fancode.conversation import ConversationManager
    from fancode.memory.instructions import load_instructions
    ...
    if config.mcp_servers:
        from fancode.mcp import MCPManager
...
async def _build_teammate_registry(
    ...
    from fancode.tools.send_message import SendMessageTool
    from fancode.tools.synthetic_output import SyntheticOutputTool
    from fancode.tools.task_create import TaskCreateTool
    from fancode.tools.task_get import TaskGetTool
    from fancode.tools.task_list import TaskListTool
    from fancode.tools.task_update import TaskUpdateTool
    from fancode.worktree import WorktreeManager
...
async def _run_teammate(team_name: str, agent_name: str) -> None:
    ...
    from fancode.teams.spawn_inprocess import LEAD_NAME, spawn_inprocess_teammate
    ...
    checker = PermissionChecker(
        ...
        sandbox=PathSandbox(work_dir),
        rule_engine=RuleEngine(
            user_rules_path=Path.home() / ".fancode" / "permissions.yaml",
            project_rules_path=Path(work_dir) / ".fancode" / "permissions.yaml",
            local_rules_path=Path(work_dir) / ".fancode" / "permissions.local.yaml",
        ),
