...
from fancode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from fancode.conversation_pairing import REJECTED_TOOL_RESULT
from fancode.conversation import ThinkingBlock as ConvThinkingBlock
from fancode.memory.auto_memory import MemoryManager
from fancode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from fancode.hooks import HookContext, HookEngine, ToolRejectedError
from fancode.hooks.engine import HookNotification
from fancode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from fancode.tools import ToolRegistry
from fancode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _announce_deferred_tools(self, conversation: ConversationManager) -> None:
        ...
        from fancode.mcp.loading_strategy import McpLoadingMode
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".fancode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".fancode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from fancode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(
        ...
        from fancode.context.manager import (
