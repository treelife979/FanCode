...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.fancode/FANCODE.md, ~/.fancode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 FANCODE.md、AGENTS.md
       和 .fancode/FANCODE.md
    3. workDir/FANCODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".fancode" / "FANCODE.md")
    _add(home / ".fancode" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add(d / "FANCODE.md")
        _add(d / "AGENTS.md")
        # .fancode/ 下的同名文件：想让指令进 .gitignore 的项目放这里
        _add(d / ".fancode" / "FANCODE.md")

    # 3. 本地覆盖
    _add(root / "FANCODE.local.md")
