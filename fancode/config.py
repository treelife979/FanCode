...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".fancode" / "config.yaml",
        cwd / ".fancode" / "config.yaml",
        cwd / ".fancode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .fancode/config.yaml "
            "in project or ~/.fancode/config.yaml"
        )
    return merged
