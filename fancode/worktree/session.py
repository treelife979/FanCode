...
def _session_path(fancode_dir: Path) -> Path:
    return fancode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(fancode_dir)
    if session is None:
...
def load_worktree_session(fancode_dir: Path) -> WorktreeSession | None:
    path = _session_path(fancode_dir)
    if not path.exists():
        return None
    try:
