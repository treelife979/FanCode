...
from fancode.commands.completion import CompletionPopup
from fancode.commands.handlers import register_all_commands
from fancode.config import MCPServerConfig, ProviderConfig
from fancode.hooks import HookContext, HookEngine, load_hooks
from fancode.conversation import ConversationManager, Message
from fancode.mcp import ConnectResult, MCPManager
from fancode.mcp.tool_wrapper import mcp_tool_name_prefix
from fancode.memory import (
    MemoryManager,
    Session,
    SessionManager,
    RecallResult,
...
from fancode.permissions import (
...
from fancode.agents.task_manager import TaskManager
from fancode.agents.trace import TraceManager
from fancode.agents.notification import inject_task_notifications
from fancode.commands.handlers.tasks import create_tasks_command
from fancode.skills.executor import SkillExecutor
from fancode.skills.loader import SkillLoader
from fancode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from fancode.tools import ToolRegistry, create_default_registry
from fancode.tools.agent_tool import AgentTool
from fancode.tools.ask_user import AskUserEvent, AskUserTool
from fancode.tools.impl.tool_search import ToolSearchTool
from fancode.tools.mcp_call import McpCallTool
from fancode.tools.install_skill import InstallSkillTool
from fancode.tools.load_skill import LoadSkill
from fancode.worktree.cleanup import start_stale_cleanup_task
from fancode.worktree.manager import WorktreeManager
from fancode.commands.handlers.worktree import create_worktree_command
from fancode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".fancode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".fancode" / "history"
...
class FanCodeApp(App):
    ...
    def __init__(
        ...
        from fancode.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        t = RichText()
        t.append("FanCode", style="bold color(99)")
        t.append("  /  terminal coding assistant\n", style="color(242)")
        if model:
            t.append("model  ", style="bold color(99)")
            t.append(model, style="color(242)")
        if work_dir:
            if model:
                t.append("  ·  ", style="color(242)")
            t.append("dir  ", style="bold color(99)")
    ...
    def on_mount(self) -> None:
        self.register_theme(_FANCODE_THEME)
        self.theme = "fancode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".fancode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".fancode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".fancode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from fancode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.fancode/config.yaml",
                        f"{work_dir}/.fancode/permissions.local.yaml",
                    ],
        ...
        from fancode.filehistory import FileHistory
        ...
        from fancode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from fancode.config import WorktreeConfig
        ...
        from fancode.tools.enter_worktree import EnterWorktreeTool
        from fancode.tools.exit_worktree import ExitWorktreeTool
        ...
        from fancode.tools.team_create import TeamCreateTool
        from fancode.tools.team_delete import TeamDeleteTool
        ...
        from fancode.commands.handlers.trace import create_trace_command
        ...
        from fancode.tools.send_message import SendMessageTool
        from fancode.tools.synthetic_output import SyntheticOutputTool
        from fancode.tools.task_stop import TaskStopTool
        ...
        if self._enable_coordinator_mode:
            from fancode.agents.tool_filter import apply_coordinator_filter
    ...
    async def _prefetch_relevant_memories(self, query: str) -> RecallResult:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from fancode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from fancode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from fancode.plan_dialog import InlinePlanWidget, PlanChoice
        from fancode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from fancode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from fancode.permission_dialog import InlinePermissionWidget
    ...
    async def _init_mcp(self) -> None:
        ...
        if self._selected_provider is not None:
            from fancode.mcp.loading_strategy import decide_and_apply
